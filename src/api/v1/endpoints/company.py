...
Preserves original route paths under /v1/company/.
"""
...
async def create_customer_route(
    ...
    from Models.db.schemas import CustomerSchema

    try:
        user = (
            db.query(models.TenantInfo)
            .filter(models.TenantInfo.ContactEmail == customer.get("Contact_Email"))
            .first()
        )
        if user is not None:
            return JSONResponse(
        ...
        user = (
            db.query(models.TenantInfo)
            .filter(models.TenantInfo.ShortName == customer.get("Company_ShortName"))
            .first()
        )
        if user is not None:
            return JSONResponse(
        ...
        new_customer = models.TenantInfo(
            ...
            TenantName=customer.get("Company_Name"),
            ContactName=f"{customer.get('Contact_FirstName')} {customer.get('Contact_LastName')}",
            ContactEmail=customer.get("Contact_Email"),
            ContactPhoneNumber=customer.get("Contact_PhoneNumber"),
            ShortName=customer.get("Company_ShortName"),
            SchemaName=f"db_saciahub_sch_tenant_{customer.get('Company_ShortName')}",
            PortalURL=f"{customer.get('Company_ShortName')}.saciahub.com",
...
async def activate_customer_route(
    ...
    try:
        ...
        result = db.execute(select_query, {"Email": user.ContactEmail}).fetchone()
        ...
        db.rollback()
        logger.error(f"Customer activate database error: {str(e)}")
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"message": str(e)},
        )
    except Exception as e:
        logger.error(f"Customer activate error: {str(e)}")
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"message": str(e)},
        )
...
async def reset_password_route(
    ...
    try:
        ...
        new_password_hash = get_password_hash(data.get("new_password"))
        update_query = text(
            f"""
        ...
        db.commit()
        portal = data.get("Company_Shortname")
        auth = UserAuthManager(db, portal)
        if user_row.get("IsTwoFA") is True:
            two_fa_token = await auth.generate_2fa_token(user_row["Email"], "tfa")
            await auth.send_2fa_email(data.get("Username"), two_fa_token)
            return {"message": "2FA verification required"}
...
async def upload_csv(
    ...
    try:
        if tenant_data is not None:
            for i in range(len(tenant_data)):
                new_customer = models.TenantInfo(
                    ...
                    BillingContactPhoneNumber=str(tenant_data.BillingContactPhoneNumber[i]),
                    ...
                    BillingAddressZipcode=str(tenant_data.BillingAddressZipcode[i]),
                ...
                mgr = SchemaManager(shortname)
                mgr.create_schema_and_tables(schemaname)
        ...
        if vendor_data is not None:
            vendor_table_name = f"{schemaname}.tb_{shortname}_vendor_info"
            for i in range(len(vendor_data)):
                ...
                db.execute(
                    insert_query,
                    {
                        ...
                        "VendorContactPhoneNumber": str(vendor_data.VendorContactPhoneNumber[i]),
                        ...
                        "VendorBillingContactNumber": str(vendor_data.VendorBillingContactNumber[i]),
                        "VendorBillingContactName": vendor_data.VendorBillingContactName[i],
                        "VendorBillingAppName": vendor_data.VendorBillingAppName[i],
                        "VendorBillingStreetName": vendor_data.VendorBillingStreetName[i],
                        ...
                        "VendorBillingPhoneNumber": str(vendor_data.VendorBillingPhoneNumber[i]),
                        ...
                        "VendorProjectExpiryIn30Days": vendor_data.VendorProjectExpiryIn30Days[i] == "TRUE",
                        "VendorProjectExpiryIn60Days": vendor_data.VendorProjectExpiryIn60Days[i] == "TRUE",
                        "VendorProjectExpiryIn90Days": vendor_data.VendorProjectExpiryIn90Days[i] == "TRUE",
                        "VendorProjectExpiryIn180Days": vendor_data.VendorProjectExpiryIn180Days[i] == "TRUE",
                        "VendorImmigrationSupport": vendor_data.VendorImmigrationSupport[i] == "TRUE",
                        "VendorSecurityClearance": vendor_data.VendorSecurityClearance[i] == "TRUE",
                        "TenantUUID": tenant_uuid,
                    },
                )
                db.commit()
        ...
        if client_data is not None:
            client_table_name = f"{schemaname}.tb_{shortname}_client_info"
            client_uuid_map = {}
            for i in range(len(client_data)):
                ...
                db.execute(
                    insert_query,
                    {
                        ...
                        "ClientContactPhoneNumber": str(client_data.ClientContactPhoneNumber[i]),
                        ...
                        "ClientBillingContactNumber": str(client_data.ClientBillingContactNumber[i]),
                        "ClientBillingContactName": client_data.ClientBillingContactName[i],
                        "ClientBillingAppName": client_data.ClientBillingAppName[i],
                        "ClientBillingStreetName": client_data.ClientBillingStreetName[i],
                        ...
                        "ClientBillingPhoneNumber": str(client_data.ClientBillingPhoneNumber[i]),
                        ...
                        "ClientBillableTotalHours": float(client_data.ClientBillableTotalHours[i]),
                        "ClientBillablePayout": float(client_data.ClientBillablePayout[i]),
                        "ClientImmigrationSupport": client_data.ClientImmigrationSupport[i] == "TRUE",
                        "ClientSecurityClearance": client_data.ClientSecurityClearance[i] == "TRUE",
                        "TenantUUID": tenant_uuid,
                    },
                )
                db.commit()
        ...
        if project_data is not None:
            ...
            for i in range(len(project_data)):
                ...
                if client_uuid is None:
                    raise ValueError(f"Client UUID not found for client name: {client_name}")
                ...
                db.execute(
                    insert_query,
                    {
                        ...
                        "ProjectTimesheetBuckets": project_data.ProjectTimesheetBuckets[i],
        ...
        if user_data is not None:
            ...
            for i in range(len(user_data)):
                ...
                user_manager_uuid = manager_assignment.get(user_data.Username[i])
                user_client_rep_uuid = client_rep_assignment.get(user_data.Username[i])
                ...
                db.execute(
                    insert_query,
                    {
                        ...
                        "User_bgv": user_data.User_bgv[i] == "TRUE",
        ...
        if settings_data is not None:
            settings_table_name = f"{schemaname}.tb_{shortname}_tenant_settings"
            for i in range(len(settings_data)):
                ...
                db.execute(
                    ...
                    {
                        "TenantUUID": tenant_uuid,
                        "TimeOffTemplates": json.dumps(settings_data.TimeOffTemplates[i]),
                    },
                )
                db.commit()
        ...
        if violations_data is not None:
            ...
            for i in range(len(violations_data)):
                insert_query = text(
                    f"""
                ...
                db.execute(
                    insert_query,
                    {
                        ...
                        "Violation_Attachment_URL": violations_data.Violation_Attachment_URL[i],
                    },
                )
                db.commit()
        ...
        db.rollback()
        logger.error(f"Pending rollback error: {e}")
    except SQLAlchemyError as e:
        db.rollback()
        raise e
    except Exception as e:
